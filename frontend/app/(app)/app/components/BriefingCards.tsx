...
export default function BriefingCards({ collectionName, domainContext }: Props) {
  ...
  if (loading) return (
    <div className="flex flex-col items-center gap-3 py-24 text-slate-700">
      <svg className="h-6 w-6 animate-spin text-amber-600" viewBox="0 0 24 24" fill="none">
        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4l3-3-3-3v4a8 8 0 00-8 8h4z"/>
      </svg>
      <p className="text-sm text-slate-700">AI가 데이터를 분석하는 중...</p>
    </div>
  );

  if (error) return <p className="py-10 text-center text-sm text-red-700">{error}</p>;

  return (
    <div>
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-slate-900">오늘의 브리핑</h2>
        <p className="text-sm text-slate-600">업로드된 데이터 기반 AI 분석 요약</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        {cards.map((card) => {
          const style = CARD_STYLE[card.id] ?? { border: "border-slate-200", glow: "from-slate-50", labelColor: "text-slate-700", icon: null };
          return (
