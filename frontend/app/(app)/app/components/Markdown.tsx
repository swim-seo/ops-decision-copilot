  ...
  while (match !== null) {
    ...
    if (match[1] !== undefined) {
      ...
      nodes.push(
        <code
          key={key}
          className="bg-rule2 px-1 py-0.5 font-data text-[0.85em] text-ink"
        >
          {match[2]}
        </code>
      );
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {blocks.map((block, i) => {
        ...
        if (block.type === "ul") {
          return (
            <ul key={key} className="list-disc space-y-1 pl-5 marker:text-ink3">
        ...
        if (block.type === "ol") {
          return (
            <ol key={key} className="list-decimal space-y-1 pl-5 marker:text-ink3">
              {block.items.map((item, j) => (
                <li key={`${key}-${j}`}>{renderInline(item, `${key}-${j}`)}</li>
              ))}
            </ol>
