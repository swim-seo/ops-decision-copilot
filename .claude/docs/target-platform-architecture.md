...
## 7. 설계 결정 (Codex 리뷰 완료 · 2026-07-18)

> 전문: [`research/codex-design-review.md`](research/codex-design-review.md)

- [x] **Q1 LangGraph vs 직접 구현 → LangGraph 채택.** 공고가 "Agentic Workflow"+프레임워크 경험을 명시 → 실무 스택 친숙도 시그널이 포폴에 +. 직접 루프는 원리는 증명하나 시그널 약하고 유지비만 늘어남.
- [x] **Q2 KG 영속성 → 도메인별 JSON blob 저장.** 조회 패턴(도메인당 그래프 통째 로드 + 간헐 2-hop)에 부합, 단순·저비용. **전환 기준**: 단일 도메인 그래프 >~20만 노드/100만 엣지 또는 2-hop 부하 ~10 QPS+ → 정규화 노드/엣지 테이블+인덱스.
- [x] **Q3 부서 가중치 RAG → 검색 시점 계산.** 부서 프로파일 수 적고 가중치 자주 바뀜 → 사전계산은 운영 부담만. **전환 기준**: 노드 >~100만 또는 ~20 QPS+ 로 지연 체감 시 → 비동기 재계산 사전점수.
- [x] **Q4 비동기 큐 → Supabase 큐 테이블(FOR UPDATE SKIP LOCKED + 워커)로 충분.** 포폴에 "적정 단순성+실무 패턴" 어필. **전환 기준**: 동시 워커 5+, 재시도/지연/워크플로 복잡도 증가, 처리량 수백 잡/일 초과 시 → Celery+Redis.

### Codex가 지적한 리스크 (반영 필요)
1. 🔴 **스프린트 순서 리스크(핵심)**: 스프린트1 LangGraph 데모를 "재시작 시 소실되는 in-memory KG" 위에 얹으면 데모 신뢰성이 깨짐 (스프린트2 영속성 이전). → **완화**: 스프린트1 범위에 **고정 데모데이터 자동 재적재(bootstrap) 단계**를 포함.
2. **설계 누락**: RAG 평가 지표(precision/recall·지연·비용) 없음, KG 재빌드/중복 upsert 멱등성 없음, 비용 가드레일 없음, 테스트 전략 없음.
3. **보안 데모 신뢰성**: RLS off + service key 사용은 "신뢰 가능한" 데모에 치명적 → 최소한 설계에 "왜 위험하고 어떻게 고칠지" 명시.

**종합 판정**: 설계는 **스프린트1 착수 가능(as written)**. 단 **조건 1개** — 스프린트1 범위에 데모데이터 bootstrap 자동재적재를 포함해 in-memory KG 재시작 문제가 데모를 훼손하지 않게 할 것. (스프린트 재정렬은 불필요, 스프린트1 데모 하드닝만 요구.)
...
| 2026-07-18 | Codex 리뷰 반영 — 4개 질문 결정 확정(LangGraph/JSON blob/쿼리시점/Supabase큐) + 스프린트1에 데모 bootstrap 조건 추가 |
