...
| 컬렉션 슬러그에 유니코드 글자를 허용 | 한글을 지우면 남는 게 없어 모든 한글 도메인이 `domain_default` 로 뭉쳤다. 슬러그는 Supabase text 컬럼과 URL 쿼리로만 흐르고 프론트가 `encodeURIComponent` 를 씌우므로 한글이 안전하다. 해시 접미사보다 사람이 읽을 수 있다 | 전부 해시(`domain_3f9a21` — UI 에 그대로 노출돼 못 알아봄) / 로마자 전사(의존성 추가 + 되돌릴 수 없음) | 2026-08-03 |
| 오염된 컬렉션을 행 단위로 지우는 대신 새 컬렉션에 재적재 | `FACT_ORDER`·`MST_SUPPLIER` 처럼 파일명이 뷰티와 supply_chain 양쪽에 있어 이름만으로는 어느 쪽 행인지 못 가린다. 재적재는 파괴적이지 않고 결과가 확정적이다 | 파일명으로 골라 삭제(오분류 위험) | 2026-08-03 |
